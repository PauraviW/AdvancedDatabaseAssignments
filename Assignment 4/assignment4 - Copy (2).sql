...
\qecho 'delete constraint on course table'
...
\qecho 'delete foreign key check on hastaken table'
\qecho ''
\qecho 'if the attribute is present in the hastaken table and if it is deleted from either the student or the course table then its entry should get deletd from the hastaken tabke too.'
insert into Student values(3, 'Rohan', 'Physics');
insert into Student values(5, 'Vivian', 'Quantum Physics');

insert into Student values(8, 'Rahul', 'Civics');
insert into hastaken values(3, 1003);
insert into hastaken values(3, 1004);
create function delete_from_hastaken_sid() returns trigger as 
$$
	begin
	if old.sid in(select h.sid from hastaken h) then
...
create trigger delete_sid_hastaken
before delete on student
for each row
execute procedure delete_from_hastaken_sid();

\qecho ' current state of student table'
table student;
\qecho ''
\qecho ' current state of hastaken table'
table hastaken;
\qecho ''
\qecho ' delete from student value where sid is not present in the hastaken table(sid = 2)'
delete from student where sid = 2;
\qecho ''
\qecho ' delete from student value where sid is  present in the hastaken table(sid = 1)'

delete from student where sid = 1;
\qecho ''
\qecho ' updated status of student and hastaken tables'
table student;
table hastaken;

\qecho 'delete check for course table'
insert into course values(1007, 'DBMS', 5, 20);
insert into course values(1015, 'Literature', 5, 20);
insert into hastaken values(8, 1007);
insert into hastaken values(5, 1007);
\qecho ''
create function delete_from_hastaken_cno() returns trigger as 
$$
	begin
	if old.cno in(select h.cno from hastaken h) then
...
create trigger delete_cno_hastaken
before delete on course
for each row
execute procedure delete_from_hastaken_cno();
...
table hastaken;
\qecho ''
\qecho ' delete from course value where cno is not present in the hastaken table(cno = 1015)'
delete from course where cno = 1015;
\qecho ' current state of course table'
table course;
\qecho ''
\qecho ' current state of hastaken table'
...
\qecho ''
\qecho ' delete from course value where cno is  present in the hastaken table(cno = 1007)'

delete from course where cno = 1007;
\qecho ''
\qecho ' updated status of course and hastaken tables'
table course;
table hastaken;
...
$$ 
	begin
		raise exception 'Delete not permitted on hastaken';
		return null;
	end;
...
delete from hastaken where sid = 3;
\qecho ''
table hastaken;
\qecho ''
...
\qecho ' Now set the enrollment status to false and we can perform addition and insertion on the Course table'
delete from enroll;
update enrollmentstarted set value = FALSE;
\qecho 'insert'
table course;
...
\qecho ''
\qecho 'question 2'
\qecho 'Trigger to let a student enroll only if he/she has taken all the prerequisite courses for that course'
\qecho ''
...
--\qecho 'drop on the waitlist table'

-- create function update_position_waitlist() returns trigger as 
-- $$
-- 	declare
-- 	pos int;
-- 	begin
	
-- 		pos = (select position from waitlist where sid = old.sid and cno = old.cno);
-- 		update waitlist set position = position - 1 where cno = old.cno and position > pos;
-- 	return old;
-- 	end;
-- $$ language 'plpgsql';
-- create trigger remove_from_wailist
-- before delete on waitlist
-- for each row
-- execute procedure update_position_waitlist();
...
-- \qecho 'Case 2: In this if a student is enrolled in multiple courses then values of students waitlisted after him for all the courses should get updated'
-- \qecho ' In this case 107 is enrolled in a multiple courses, so if I delete 107 from the waitlist table, the position of the other students after him for all the courses that he is enrolled in should get updated'

-- delete from waitlist where sid = 107;
-- table waitlist;

-- \qecho 'As we can see that all the values get correctly updated'
\qecho ''
...
\qecho 'question 3'
-- I create this table for storing the final values of the expected value and standard deviation
create table expec_Val(n int, et float, vt float);
-- I create this table just to trigger the throw event 
create table T(n int, sum int);

-- this function implements the standard deviation and expected value incrementally 
...
-- This is the trigget that calls the procedure to calculate the expected values
create trigger calc_exp_val 
before insert on T
for each row
execute procedure expected_val();

-- This is the function that throws and incrementally calculates the results n times. I put the results in the record variable and return it.
...
select n, Expected_value, Standard_deviation  from expt(100) expt(n int, Expected_value float, Standard_deviation float);
drop table t;
